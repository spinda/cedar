...
use crate::parser::node::WithSourceInfo;

pub(crate) type RawLocation = usize;
pub(crate) type RawToken<'a> = lalr::lexer::Token<'a>;
pub(crate) type RawUserError = WithSourceInfo<String>;
...
#[cfg_attr(feature = "unstable-miette", derive(Diagnostic))]
pub enum ParseError {
    /// Error from the CST parser
    #[cfg(not(feature = "unstable-to-cst-err"))]
    #[error("{0}")]
    #[cfg_attr(
        feature = "unstable-miette",
        diagnostic(code(cedar_policy_core::parser::to_cst_err))
    )]
    ToCST(String),
    /// Error from the CST parser
    #[cfg(feature = "unstable-to-cst-err")]
    #[error(transparent)]
    #[cfg_attr(feature = "unstable-miette", diagnostic(transparent))]
    ...
    #[cfg_attr(
        not(feature = "unstable-miette"),
        error("error while {context}: {}", MultipleParseErrors(errs))
    )]
    #[cfg_attr(feature = "unstable-miette", error("error while {context}"))]
    WithContext {
...
impl ParseError {
    ...
    pub(crate) fn from_raw_parse_err(err: RawParseError<'_>) -> Self {
        let to_cst_err = ToCSTError::from_raw_parse_err(err);
        #[cfg_attr(not(feature = "unstable-miette"), allow(unused_mut))]
        let mut err_string = to_cst_err.to_string();

        #[cfg(feature = "unstable-miette")]
        if let Some(labeled_spans) = to_cst_err.labels() {
            for labeled_span in labeled_spans {
                if let Some(label) = labeled_span.label() {
                    err_string.push_str("; ");
                    err_string.push_str(label);
                }
            }
        }

        Self::ToCST(err_string)
    }

    #[cfg(feature = "unstable-to-cst-err")]
    pub(crate) fn from_raw_parse_err(err: RawParseError<'_>) -> Self {
        ToCSTError::from_raw_parse_err(err).into()
    }
    ...
    #[cfg(feature = "unstable-to-cst-err")]
    pub(crate) fn from_raw_err_recovery(recovery: RawErrorRecovery<'_>) -> Self {
        ToCSTError::from_raw_err_recovery(recovery).into()
    }
}
...
#[cfg(not(feature = "unstable-to-cst-err"))]
use unstable::ToCSTError;
#[cfg(feature = "unstable-to-cst-err")]
pub use unstable::ToCSTError;

/// Like [`ParseErrors`], but you don't have to own the `Vec`
#[cfg(not(feature = "unstable-miette"))]
#[derive(Debug, Error)]
#[deprecated(note = "use `ParseErrors` directly instead")]
